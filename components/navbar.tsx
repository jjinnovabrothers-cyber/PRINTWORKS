...
export function Navbar({ onOpenService }: { onOpenService: (id: number) => void }) {
    ...
    <header className="fixed top-0 z-50 w-full border-b border-[rgba(34,197,94,0.15)] bg-[rgba(5,8,5,0.92)] backdrop-blur-md">
      <div className="mx-auto flex h-16 max-w-7xl items-center justify-between px-4 lg:px-8">
        ...
        <nav className="hidden items-center gap-1 md:flex" aria-label="Navegacion principal">
          {NAV_ITEMS.map((item) => (
            <button
              ...
              className="rounded-full px-4 py-2 text-sm font-medium tracking-wide text-[#d1d5db] transition-colors hover:bg-[rgba(34,197,94,0.1)] hover:text-white"
            >
              {item.label}
            </button>
          ))}
        ...
        <button
          ...
          aria-label="Menu"
        >
          <span className={`h-0.5 w-6 bg-[#22c55e] transition-transform ${mobileOpen ? "translate-y-2 rotate-45" : ""}`} />
          <span className={`h-0.5 w-6 bg-[#22c55e] transition-opacity ${mobileOpen ? "opacity-0" : ""}`} />
          <span className={`h-0.5 w-6 bg-[#22c55e] transition-transform ${mobileOpen ? "-translate-y-2 -rotate-45" : ""}`} />
        </button>
      </div>
      ...
      {/* Mobile menu */}
      {mobileOpen && (
        <nav className="border-t border-[rgba(34,197,94,0.1)] bg-[rgba(5,8,5,0.98)] px-4 pb-4 pt-2 md:hidden">
          {NAV_ITEMS.map((item) => (
            <button
              key={item.id}
              onClick={() => scrollTo(item.id)}
              className="block w-full rounded-lg px-4 py-3 text-left text-sm font-medium text-[#d1d5db] hover:bg-[rgba(34,197,94,0.1)] hover:text-white"
            >
              {item.label}
            </button>
          ))}
